...
A screening synopsis delivered as an HTML slide deck, routed through an advisor who asked to see the candidate's thinking before an introductory meeting with KUKA's Global CEO. The advisor reads it alone first (must be self-standing), and the candidate later walks it live (arrow-driven reveals). Fifteen screens in four acts.
...
- **Overview:** dedicated **master navigation button** (persistent, top-right) **and** the `O` key / `Esc` — opens a grid of all 16 screens with titles; click any to jump. This is an explicit requirement, not optional.
- **Replay:** `R` — replays the current screen's animation
- **Jump:** `Home` / `End`
- **Progress:** slim rail with 16 clickable segments + `NN / 16` counter
...
| `jointStagger` | per-joint power-up with servo easing (slight overshoot, settle) | cover arm only |
