 ...
 * drawing tip, and the title fades up as it lands — none of it consumes a
 * step. Steps 1-3 (subtitle, credential block, settle) are ordinary
 * advance-driven reveals. `R` replays the whole base sequence; onLeave
 * cancels every timer and rAF so nothing survives navigation.
 *
 * Arm geometry AND the sequence timing are read from arm-geometry.js, not
 * redefined here — screen 16 reuses both so the bookend cannot drift.
 *
...
(function () {
  ...
  function jointGroup(name, childHTML) {
    ...
    return `<g class="joint" data-joint="${name}"
      ...
      <line class="arm-link" x1="0" y1="0" x2="${len}" y2="0"></line>
      <circle class="joint-dot" cx="0" cy="0" r="7"></circle>
      ${isTip ? `<circle class="joint-dot end-effector-dot" cx="${len}" cy="0" r="8"></circle>` : ""}
      ${childHTML}
    </g>`;
  }
    ...
    return `
      <section class="screen screen--cover">
        ...
        <div class="cover-title-block">
          ...
          <p class="subtitle step" data-step="1">A global read on the physical-AI inflection — and why India is not a market to serve, but the engine of the answer.</p>
  ...
  // --- step 3: settle — single pulse, no loop ---------------------------

  function playPulse(root) {
    if (isReduced) return; // spec: no pulse, no glow-follow under reduced motion
    const pulse = root.querySelector(".cover-pulse");
    ...
    pulseController = { cancel: () => anim.cancel() };
    controllers.push(pulseController);
  }
