 ...
 * like) and breaks at step 5: seven strands stop at ~60% across and fray
  ...
  function render() {
    ...
    return `
      <section class="screen screen--paradox">
        ...
        <p class="pdx-closing">Demand is not the constraint. Capacity to capture it is.</p>
      </section>
    `;
  }
  ...
  function revealStep(root, n) {
    ...
    if (n === 4) {
      root.querySelectorAll(".pdx-fact").forEach((f, i) => {
        ...
        else schedule(() => f.classList.add("is-visible"), i * 140);
      });
    }
    if (n === 5) breakChannel(root);
    if (n === 6) {
