...
- **Overview:** dedicated **master navigation button** (persistent, top-right) **and** the `O` key / `Esc` — opens a grid of all 15 screens with titles; click any to jump. This is an explicit requirement, not optional.
- **Replay:** `R` — replays the current screen's animation
- **Jump:** `Home` / `End`
- **Progress:** slim rail with 15 clickable segments + `NN / 15` counter
