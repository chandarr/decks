...
- **Approximate positions** (fractions of plot width × height, origin bottom-left):
  - Traditional leaders — `(0.28, 0.80)` · vector → right, length 0.16
  - Chinese OEMs — `(0.42, 0.55)` · vector → up-right, length 0.15
  - Modern mfg startups — `(0.30, 0.40)` · vector → left-up (toward hardware depth), length 0.14
  - Humanoids — `(0.62, 0.22)` · vector → up-left, length 0.18
  - Physical-AI software — `(0.80, 0.15)` · vector → up-left, length 0.20
  - **KUKA marker** — `(0.34, 0.86)` · vector → right, length **0.06**
...
```js
CLUSTERS = [
  { id:"traditional", label:"TRADITIONAL LEADERS", pos:[0.28,0.80], vec:[1,0.15], len:0.16,
    members:[...], relevance:"...", tier:"confirmed" },
  ...
]
KUKA_MARKER = { pos:[0.34,0.86], vec:[1,0.05], len:0.06, tier:"confirmed" }
```
