 ...
 */
(function () {
  // ---- plot geometry (viewBox 0 0 1440 400) --------------------------
  // The viewBox aspect deliberately matches the band the plot is laid into
  // (~1440x390 on the design canvas). It used to be 960x500, so `meet` could
  // only ever fill half the available width — the plot rendered as a small
  // island in a wide empty strip, and its 11px labels scaled down to ~9px.
  const ML = 56, MT = 18, PW = 1350, PH = 336; // margins / plot width+height
  ...
  const AXIS_BOTTOM = MT + PH;
  const AXIS_TOP = MT;
  const AXIS_RIGHT = ML + PW - 16; // axis stops just short of the plot edge
  const ARROW_TIP = AXIS_RIGHT + 4;
    ...
    return `
      <section class="screen screen--inflection">
        ...
        <div class="infl-plot-wrap">
          <svg class="infl-plot-svg" viewBox="0 0 1440 400" preserveAspectRatio="xMidYMid meet" aria-hidden="true">
            ...
            <line class="infl-axis" x1="${ML}" y1="${AXIS_BOTTOM}" x2="${AXIS_RIGHT}" y2="${AXIS_BOTTOM}"></line>
            <path class="infl-axis" d="M ${AXIS_RIGHT - 5} ${AXIS_BOTTOM - 5} L ${ARROW_TIP} ${AXIS_BOTTOM} L ${AXIS_RIGHT - 5} ${AXIS_BOTTOM + 5}"></path>
            ${ticks}
            ...
            <line class="infl-threshold" x1="${ML}" y1="${THRESHOLD_Y}" x2="${AXIS_RIGHT}" y2="${THRESHOLD_Y}"></line>
            <text class="infl-threshold-label" x="${AXIS_RIGHT}" y="${THRESHOLD_Y - 8}" text-anchor="end">industrial reliability threshold</text>
