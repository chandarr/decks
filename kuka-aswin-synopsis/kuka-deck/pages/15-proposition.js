...
(function () {
  ...
  // lengths are untouched — only the rendering compensates for the scale. The
  // housing outlines are NOT compensated: they are the robot's actual volume,
  // so they scale with it and the arm simply reads smaller, not stubbier.
  const STROKE = +(G.weights.link / S).toFixed(2);
  const EDGE = +(G.weights.edge / S).toFixed(2);
  ...
  const BLOCKS = [
    ...
    },
    {
      ...
      body: "RaaS fleets as instrumented nodes. The deployment data accrues to KUKA, not to a third-party integrator.",
    },
    {
      verb: "EXPORT",
  ...
  // Step map. Step 1 fires the whole opening at once — the extension
  // trajectory, the closing statement and the recommendation land on a single
  // advance, so the screen arrives composed rather than assembling itself over
  // three clicks. The four stages then come one per click, which is where the
  // presenter actually talks; the close takes the last two.
  const FIRST_BLOCK_STEP = 2;
  const FINAL_STEP = FIRST_BLOCK_STEP + BLOCKS.length; // 6
  const SIGNATURE_STEP = FINAL_STEP + 1; // 7
  ...
  function jointGroup(name, childHTML) {
    ...
    return `<g class="joint" data-joint="${name}" transform="rotate(${a.final})">
      <path class="arm-body" fill="url(#prop-link-body)" stroke-width="${EDGE}" d="${G.linkBodyPath(name)}"></path>
      ...
      ${isTip ? `${gripper()}<circle class="joint-dot end-effector-dot" cx="${len}" cy="0" r="${TIP_R}"></circle>` : ""}
      ${childHTML}
    </g>`;
  }
  function armChain() {
      ...
      <section class="screen screen--proposition">
        <svg class="prop-svg" viewBox="0 0 1600 900" preserveAspectRatio="xMidYMid meet" aria-hidden="true">
          <defs>
            ...
            <!-- Same stops as the cover's cover-link-body: runs across each
                 link's thickness in the link's own rotated frame, so every
                 housing stays lit from its own top edge. -->
            <linearGradient id="prop-link-body" x1="0" y1="0" x2="0" y2="1">
        ...
        <div class="prop-statement">
          ...
          <div class="prop-closing reveal">
            <p class="prop-closing-line">KUKA does not need to become someone else to harness the Physical AI revolution.</p>
          ...
          <p class="prop-recommendation reveal">Rebuild India as both a development and deployment base, not only a market — and let the playbook it produces carry the global position.</p>
        </div>

        <div class="prop-blocks">${blocks}</div>

        <!-- Occupies the lower-left quadrant, which the arm vacates once the
             trajectory has flattened into the spine. Placed level with the four
             blocks and to their left, so it reads as the synthesis of them
             rather than a fifth move. -->
        <div class="prop-left">
          <p class="prop-left-title reveal">KUKA can win the decade ahead.</p>
          <p class="prop-left-sub reveal">Not in pieces. These four moves compound only when the global business is read as one system &mdash; taken separately each is a local cost, taken together they are a position no competitor can assemble.</p>
        </div>
  ...
  // Index of the stage this step reveals, or -1 if the step is not a stage.
  function blockIndex(n) {
    const i = n - FIRST_BLOCK_STEP;
    return i >= 0 && i < BLOCKS.length ? i : -1;
  }

  function revealStep(root, n) {
    if (n === 1) {
      // All three start on the same beat. The only internal offset is the
      // recommendation waiting on its own rule to finish drawing.
      const ext = root.querySelector(".prop-ext-traj");
    ...
    const bi = blockIndex(n);
    if (bi >= 0) {
      root.querySelector(`.prop-node[data-node="${bi}"]`).classList.add("is-lit");
      root.querySelector(`.prop-block[data-block="${bi}"]`).classList.add("is-visible");
    }

    if (n === FINAL_STEP) {
      // The lower-left synthesis lands here, after all four moves are on the
      // spine — its subtitle refers to them, so it cannot precede them.
      controllers.push(window.Anim.fadeUp(root.querySelector(".prop-left-title"), { reduced: isReduced }));
      const sub = root.querySelector(".prop-left-sub");
      if (isReduced) sub.classList.add("is-visible");
      else schedule(() => controllers.push(window.Anim.fadeUp(sub, { reduced: false })), 260);
    }
    if (n === SIGNATURE_STEP) {
  ...
  function hideStep(root, n) {
    if (n === 1) {
      ...
      root.querySelector(".prop-glow-head").style.opacity = "0";
      root.querySelector(".prop-closing").classList.remove("is-visible");
    ...
    const bi = blockIndex(n);
    if (bi >= 0) {
      root.querySelector(`.prop-node[data-node="${bi}"]`).classList.remove("is-lit");
      root.querySelector(`.prop-block[data-block="${bi}"]`).classList.remove("is-visible");
    }

    if (n === FINAL_STEP) {
      root.querySelectorAll(".prop-left-title, .prop-left-sub").forEach((e) => e.classList.remove("is-visible"));
    }
    if (n === SIGNATURE_STEP) root.querySelector(".prop-signature").classList.remove("is-visible");
  }

  window.page({
    id: "15-proposition",
    title: "KUKA does not need to become someone else to harness the Physical AI revolution.",
    theme: "dark",
    steps: SIGNATURE_STEP,
    render,
    onEnter: (root, ctx) => {
      clearAll();
